...
| `sound.ts` | `tocarAcerto()`/`tocarErro()` — efeitos sonoros sintetizados via Web Audio API (sem arquivos de áudio, sem chamada de rede). Cada tela decide sozinha se chama (checando `somLigado` do `progressStore`) — o módulo não sabe nada sobre o toggle de som |
